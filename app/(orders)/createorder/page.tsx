...
export default function CreateOrderPage() {
  ...
  const selectedSupplier = mockSuppliers.find(
    (s) => s.id === shopFormData.supplierId
  );
  ...
  const handleSubmit = (status: "draft" | "pending") => {
    ...
    if (orderType === "online") {
      if (
        !onlineFormData.customer.name ||
        !onlineFormData.customer.email ||
        !onlineFormData.customer.address
      ) {
    ...
    const generateOrderNumber = () => {
      const now = new Date();
      return `${
        orderType === "online" ? "ON" : "SH"
      }-${now.getFullYear()}${Math.floor(1000 + Math.random() * 9000)}`;
    };
    const now = new Date();
    const orderdate = now.toString();
    const newOrder = {
      id: orderdate,
      ...
      expectedDate:
        orderType === "online"
          ? onlineFormData.expectedDate
          : shopFormData.expectedDate,
      status: status,
      priority:
        orderType === "online"
          ? onlineFormData.priority
          : shopFormData.priority,
      subtotal: subtotal,
      tax: tax,
      shipping: shipping,
      totalAmount: total,
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <button
          ...
          className={`group relative overflow-hidden rounded-2xl border-2 p-3 text-left transition-all duration-300 hover:-translate-y-1 ${
            orderType === "online"
        ...
        >
          <div
            className={`absolute right-0 top-0 h-32 w-32 group-hover:w-[900px] group-hover:h-[300px] translate-x-8 -translate-y-8 group-hover:translate-x-12 group-hover:-translate-y-12 rounded-full transition-all duration-500 ${
              orderType === "online" ? "bg-white/10" : "bg-emerald-500/10"
            }`}
          />
            ...
            <div>
              <h3
                className={`text-xl font-bold ${
                  orderType === "online" ? "text-white" : "text-slate-900"
                }`}
              >
                Online Order
              </h3>
        ...
        >
          <div
            className={`absolute right-0 top-0 h-32 w-32 group-hover:w-[900px] group-hover:h-[300px] translate-x-8 -translate-y-8 group-hover:translate-x-12 group-hover:-translate-y-12 rounded-full transition-all duration-500 ${
              orderType === "shop" ? "bg-white/10" : "bg-emerald-500/10"
            }`}
          />
          <div className="relative flex items-center gap-4">
            <div
            ...
            <div>
              <h3
                className={`text-xl font-bold ${
                  orderType === "shop" ? "text-white" : "text-slate-900"
                }`}
              >
                Shop Order
              </h3>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-6 rounded-2xl border border-slate-200/50 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 space-y-3">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700 capitalize">
                        {onlineFormData.shippingMethod} shipping
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {orderType === "shop" &&
                shopFormData.deliveryMethod === "delivery" && (
                  <div className="rounded-lg bg-blue-50 p-3">
                    <div className="flex items-start gap-2">
                      <Truck className="mt-0.5 h-4 w-4 text-blue-600" />
                      <div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-xs font-medium text-amber-900">
                        Expected{" "}
                        {orderType === "online" ? "Delivery" : "Receipt"}
                      </p>
